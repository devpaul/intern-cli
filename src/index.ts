...
import { acceptVersion, die, enumArg, copy, collect, intArg, exitCodeForSignal } from './util';
import * as program from 'commander';
import { getLogger, print } from './log';
import { EOL } from 'os';
...
if (!acceptVersion(internPackage.version, MIN_VERSION)) {
	die(`This command requires Intern ${ MIN_VERSION } or newer (${ internPackage.version } is installed).`);
}
...
program.helpInformation = function (this: any) {
	...
	const options = [
		'  Options:',
		'',
		String(this.optionHelp().replace(/^/gm, '    ')),
		'',
		''
	];
...
program
	...
	.on('help', function (args: string[]) {
		...
		if (command) {
			...
			if (commandName) {
				print(`unknown command: ${ commandName }${ EOL }`);
			}
...
program
	...
	.option('-b, --browser <browser>', 'browser to use for functional tests',
		enumArg.bind(null, Object.keys(BROWSERS)), 'chrome')
	.action(function (options) {
		...
		try {
			...
			let data: string = fs.readFileSync(path.join(internDir, 'tests', 'example.intern.js'), { encoding: 'utf8' });
			...
			const info: BrowserDescriptor = BROWSERS[options.browser];
			let note: string = info.note;

			if (!note) {
				note = `Note that running WebDriver tests with ${ info.name } requires ${ info.driver } to be ` +
...
program
	...
	.option('-f, --fsuites <module ID>', 'specify a functional suite to run (can be used multiple times)', collect,
		[])
	...
	.action(function () {
		...
		intern.on('close', function (code, signal) {
			if (process.exitCode == null) {
				process.exitCode = code != null ? code : exitCodeForSignal(signal);
			}
		});
	...
	.on('--help', function () {
		...
		const digdugDir = path.dirname(require('resolve').sync('digdug/Tunnel', { basedir: process.cwd() }));
